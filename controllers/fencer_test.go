...
const (
	...
	// defaultFencerRetryInterval is the retry interval when fencing operation
	// fails.
	defaultFencerRetryInterval = 5 * time.Second

	// defaultFencerTimeout is the timeout for a fencer.
	defaultFencerTimeout = 25 * time.Second
)
...
func SetupFencerTest(ctx context.Context, nsName string, node *corev1.Node, pod *corev1.Pod, pvcs []*corev1.PersistentVolumeClaim, pvs []*corev1.PersistentVolume, vas []*storagev1.VolumeAttachment, vols []storageos.Object) {
	...
	BeforeEach(func() {
		...
		err = controller.SetupWithManager(ctx, mgr, defaultWorkers, defaultFencerRetryInterval, defaultFencerTimeout)
...
func genPodForFencing(name string, namespace string, fenced bool, nodeName string, pvcs []*corev1.PersistentVolumeClaim) *corev1.Pod {
	pod := &corev1.Pod{
...
func genPVCForVolume(name string, namespace string, volName string, stos bool) *corev1.PersistentVolumeClaim {
	...
	if stos {
		pvc.Annotations[provisioner.PVCProvisionerAnnotationKey] = fencer.DriverName
	}
	return pvc
}
...
var _ = Describe("Fencing controller", func() {
	...
	Context("Pod with fencing and storageos pvc", func() {
		...
		pvcs := []*corev1.PersistentVolumeClaim{genPVCForVolume(pvcName, nsName, volName, true)}
		vas := []*storagev1.VolumeAttachment{genVA(vaName, volName, nodeName)}
		vols := []storageos.Object{genVol(volName, nsName, true)}
		pod := genPodForFencing(podName, nsName, true, nodeName, pvcs)
	...
	Context("Pod without fencing and storageos pvc", func() {
		...
		vas := []*storagev1.VolumeAttachment{genVA(vaName, volName, nodeName)}
		vols := []storageos.Object{genVol(volName, nsName, true)}
		pod := genPodForFencing(podName, nsName, false, nodeName, pvcs)
	...
	Context("Pod with fencing and multiple storageos pvcs", func() {
		nsName, nodeName, podName, volName, pvcName, vaName := genNames(randStringRunes(5))
		pvcs := []*corev1.PersistentVolumeClaim{
			genPVCForVolume(pvcName+"-1", nsName, volName+"-1", true),
			genPVCForVolume(pvcName+"-2", nsName, volName+"-2", true),
			genPVCForVolume(pvcName+"-3", nsName, volName+"-3", true),
		}
		vas := []*storagev1.VolumeAttachment{
	...
	Context("Pod with fencing and mixed pvcs", func() {
		...
		pvcs := []*corev1.PersistentVolumeClaim{
			genPVCForVolume(pvcName+"-1", nsName, volName+"-1", true),
			genPVCForVolume(pvcName+"-2", nsName, volName+"-2", false), // not StorageOS
			genPVCForVolume(pvcName+"-3", nsName, volName+"-3", true),
		}
	...
	Context("Pod with fencing and mixed pvcs with missing pv", func() {
		...
		pvcs := []*corev1.PersistentVolumeClaim{
			genPVCForVolume(pvcName+"-1", nsName, volName+"-1", true),
			genPVCForVolume(pvcName+"-2", nsName, volName+"-2", false), // not StorageOS, no PV
			genPVCForVolume(pvcName+"-3", nsName, volName+"-3", true),
		}
		// pvs := []*corev1.PersistentVolume{genPV(volName+"-2", "csi.vendor-x.com")}
		vas := []*storagev1.VolumeAttachment{
